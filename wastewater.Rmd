...
cases <- read.csv(cases) %>%
  ...
  rename(Site = "Hall_Nbhd") %>%
  filter(Site == "Lakeshore" | Residence_Hall == "Sellery") %>%
  mutate(Site = ifelse(Site == "SouthEast",
                            "UW-Sellery", "UW-Lakeshore")) %>%
...
```{r}
ggplot(case_waters %>% 
         filter(assay %in% c("Positive", "AVG","PMMoV","Pct_BCoV"))) +
  aes(Date, values, col = Site) +
