...
water_MMSD <- read_excel("data/wastewater/WW SARS-COV-2 Data V2.xlsx",
                    ...
                    sheet = 1) %>%
  rename(Comment = "...12")
water_Interceptors <- read_excel("data/wastewater/WW SARS-COV-2 Data V2.xlsx",
                    na = c("","NA","0","Not Detected","Field Parameters to be filled in", "Inhibited-to be re-ran", "#DIV/0!"),
...
water_UW_Dorms <- read_excel("data/wastewater/WW SARS-COV-2 Data V2.xlsx",
                    na = c("","NA","0","Not Detected","Field Parameters to be filled in", "Inhibited-to be re-ran", "#DIV/0!"),
                    col_types = c("text", "date", rep("numeric", 9), rep("text", 2)),
                    sheet = 3)[,-13] %>%
  rename(Comment = "...12")
water <- 
...
The MMSD sites are (I think) sewage pumping stations in the [MMSD Service Area](https://www.madsewer.org/About-Us/Service-Area) (see interactive map). P2 and P8 are closest to south side of campus.
...
```{r fig.height = 10}
ggplot(case_waters %>% 
         filter(assay %in% c("Positive", "N1", "N2", "AVG","PMMoV","Pct_BCoV"))) +
  aes(Date, values, col = Site) +
  geom_smooth(method = "loess", se = FALSE, size = 1, span = 0.5) +
  geom_point(alpha = 0.5) +
  scale_y_log10() +
