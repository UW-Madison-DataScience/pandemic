...
house <- cdc_date %>%
  ...
  ungroup
roomsize <- house %>%
  count(FAKEHALL, FAKEHOUS, fakeroom)
house <-
  left_join(house, roomsize, by = c("FAKEHALL", "FAKEHOUS", "fakeroom")) %>%
  filter(n == 2) %>%
  select(-n)
```
...
pair_house <- function(house) {
  pos_pair <- house %>%
    filter(Positive == 1) %>%
    mutate(fakeroom = room_rank(fakeroom)) %>%
  ...
  pos_room <- pos_pair %>%
    ...
    summarize(Positive = n(),
              Room = first(Date)) %>%
    ungroup %>%
    mutate(Room = rank(Room, ties.method = "random"))
  left_join(
...
ggplot(pair_hall(house %>% filter(FAKEHALL == 10))) +
  aes(Date, Room, col = Positive, group = Room) +
  geom_line() +
  geom_point() +
...
ggplot(pair_house(house %>% filter(FAKEHALL == 10))) +
  aes(Date, Room, col = factor(FAKEHOUS), group = Room) +
  geom_line() +
  geom_point() +
  theme(legend.position = "none") +
  ggtitle("Positive Collection Dates by Room Burden across Rooms in House")
...
out <- 
  bind_rows(
    map(split(house, house$FAKEHALL), pair_house),
    .id = "hall")
```
