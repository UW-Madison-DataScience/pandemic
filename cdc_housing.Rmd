...
Idea here is to just look at dates of positive cases and link cases within rooms by a horizontal line. First plot colors rooms by 1 or 2 positive cases. Other plots color rooms by house to look for patterns that cut across houses in halls.
...
pair_house <- function(house, neg = FALSE) {
  ...
  out <- 
    left_join(
      pos_pair %>% select(-Positive),
      pos_room,
      by = "fakeroom") %>%
    mutate(Positive = factor(Positive),
          students = nhouse) %>%
    count(students, PrevDate, Date, Room, RoomID, Positive, name = "count")
  if(nrow(out)) {
    out %>%
    # jitter roommates if 2
    group_by(Room) %>%
    mutate(RoomID = ifelse(count == 2, RoomID + c(-0.2,0.2), RoomID)) %>%
    ungroup %>%
    pivot_longer(PrevDate:Date, names_to = "Prev", values_to = "Date") %>%
    group_by(RoomID) %>%
    mutate(RoomShape = as.character(1:2)) %>%
    ungroup
  } else {
    out 
  }
...
```{r}
out <- pair_house(house %>% filter(FAKEHALL == 10, FAKEHOUS == 39))
```

```{r}
ggplot(out) +
  aes(Date, RoomID, col = Positive, group = RoomID, shape = RoomShape) +
  geom_line() +
  geom_point() +
...
```{r}
plot_hall <- function(house, hallID) {
...
ggplot(out) +
  ...
  ggtitle(paste("Positive Collection Dates by Room Burden across Youse Rooms in Hall", hallID))
}
```
