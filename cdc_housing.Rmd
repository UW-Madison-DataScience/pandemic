...
Subsequent plots show individual students per horizontal line (sometimes with a little jitter), with roommates adjacent to each other. Coloring highlights time course of dates, positive vs negative, or room. Only considering rooms with exactly two roommates, as other sizes might be due to room transfers.
...
```{r fig.height = 4}
ggplot(pair_house(house %>% filter(FAKEHALL == 10, grepl("39", FAKEHOUS)), TRUE)) +
  aes(Date, RoomID, col = RoomPos, group = RoomID, shape = TestResult) +
  geom_vline(xintercept = as.Date(c("2020-08-25","2020-09-01","2020-09-04","2020-09-09","2020-09-17")),
             linetype = "dashed", col = "gray") +
  geom_line() +
