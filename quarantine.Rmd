...
do_date <- function(cond, val, Date) {
  out <- Date[val]
  if(cond) {
    out <- as.Date(NA)
  }
...
house <- cdc_date %>%
  ...
  summarize(FirstPos = which.max(diff(c(0, Positive))),
            FirstPos = ifelse(FirstPos == 1 & Positive[1] == 0, NA, FirstPos), # No positives.
            FirstNeg = ifelse(is.na(FirstPos) | FirstPos > 1, 1, NA), # First negative. Or not.
            LastPos  = ifelse(is.na(FirstPos), NA, which.min(diff(c(Positive, 0)))),
            NextNeg  = ifelse(is.na(LastPos) | LastPos == n(), NA, LastPos + 1),
            LastNeg  = ifelse(!is.na(LastPos) & LastPos == n(), NA, n()),
            FirstNeg = do_date(is.na(FirstNeg), FirstNeg, Date),
            PrevNeg  = do_date(is.na(FirstPos) | FirstPos == 1, FirstPos - 1, Date),
            FirstPos = do_date(is.na(FirstPos), FirstPos, Date),
            LastPos  = do_date(is.na(LastPos), LastPos, Date),
            NextNeg  = do_date(is.na(NextNeg), NextNeg, Date),
            LastNeg  = do_date(is.na(LastNeg), LastNeg, Date),
            Positive = max(Positive),
            .groups = "drop")
```
